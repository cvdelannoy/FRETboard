...
def plot_recall_precision(confusion_df, ax):
    pr = sns.scatterplot(x='recall', y='precision', style='state', hue='category', data=confusion_df, ax=ax)
    pr.legend_.remove()
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
...
def plot_coverage_violin(eventstats_df, ax):
    sns.violinplot(x='category', y='coverage', data=eventstats_df.loc[eventstats_df.label == eventstats_df.predicted], ax=ax)
