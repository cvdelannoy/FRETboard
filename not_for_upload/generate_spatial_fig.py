...
from plotting_functions import plot_trace, plot_tdp

# pointers taken from https://stackoverflow.com/questions/31452451/importing-an-svg-file-into-a-matplotlib-figure

parser = argparse.ArgumentParser(description='Generate figures related to fret efficiency measures')
...
parser.add_argument('--means-dict', type=str, required=True)
parser.add_argument('--tdp-tsv', type=str, required=True)
...
ax_ts1 = traces_fig.add_subplot(traces_gs[0, :2])
ax_ts2 = traces_fig.add_subplot(traces_gs[1, :2], #sharey=ax_ts1, #sharex=ax_ts1,
                                )
kde_gs = gridspec.GridSpec(2, 4, figure=traces_fig, wspace=0.5, bottom=0.2)
ax_kde = traces_fig.add_subplot(kde_gs[:, 2])

tdp_gs = gridspec.GridSpec(2, 4, figure=traces_fig, wspace=0.5, bottom=0.2)
ax_tdp = traces_fig.add_subplot(tdp_gs[:, 3])
...
plot_dicts.append({
    'ylabel': 'I',
    'xlabel': 'time (s)',
    'data': dat_df.loc[:, ['f_dex_dem', 'f_dex_aem', 'i_sum']].to_numpy(),
    'label': dat_df.loc[:, 'predicted'].to_numpy(),
...
# TDP
tdp_df = pd.read_csv(args.tdp_tsv, sep='\t', header=0)
with open(args.means_dict, 'rb') as fh: efret_means = pickle.load(fh)
plot_tdp(tdp_df, efret_means, ax_tdp)
