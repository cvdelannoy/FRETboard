...
class Classifier(object):
    ...
    def get_trained_hmm(self, data_dict, bootstrap=False):
        ...
        hmm, gm_dict = self.get_untrained_hmm(data_dict)
    ...
    def convert_gmm_labels(self, labels, X, gm_dict):
        """
        Numeric labels to GMM-preclassified labels
        """
        out_labels = []
        for li, l in enumerate(labels):
            if type(l) is str:
                ...
                y = gm_dict[f's{l}'].predict(X[li - 1, :].reshape(1,-1))[0]  # li corrected for hidden starting state
                out_labels.append(f's{l}_{y}')
                # out_labels.extend([f's{l}_start', f's{l}_{y}', f's{l}_end'])
        return out_labels


    def add_boundary_labels(self, labels):
        """
        ...
        for li, l in enumerate(labels):
            if l == cur_label:
                ...
                out_labels[li-overhang_left+1:li+1] = [f'e{prev_label}{cur_label}_{i + ol_residual}' for i in range(ol_safe)]
        return ['None-start'] + out_labels + ['None-end']
    ...
    def predict(self, trace_df, hmm=None):
        ...
        trace_state_list = hmm.predict(trace_df.loc[:, self.feature_list].to_numpy(), algorithm='viterbi')[1:-1]  # todo segfaults with viterbi??
        logprob = self.trained.log_probability(trace_df.loc[:, self.feature_list].to_numpy())  # todo now requires 2 runs for logprob only!
