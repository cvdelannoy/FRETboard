...
Windows, Linux or MacOS system using pip:
...
| NetCDF-4    | Should contain at least `intensity` and `time`, where `intensity` is a 3D array of shape (number of traces, number of channels, trace duration), and the length of `time` matches the last dimension of `intensity`. | .nc       |
