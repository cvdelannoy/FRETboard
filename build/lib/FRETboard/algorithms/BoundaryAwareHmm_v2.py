...
class Classifier(object):
    ...
    def get_untrained_hmm(self, data_dict):
        ...
        for es_name in edge_states:
            ...
            hmm.add_transition(es, s2, 1.0, pseudocount=9999999)
    ...
    def get_states(self, data_dict): # todo check
        ...
        for edge in edges:
            ...
            estates_list = list()
            edge_state = pg.State(distfun(*edge), name=f'e{edge[0]}_{edge[1]}')
            edge_states[sn] = [edge_state, (f's{edge[0]}', f's{edge[1]}')]
            pg_gui_state_dict[f'{sn}'] = int(edge[1])
    ...
    def tm_from_hmm(self, hmm, state_order_dict):
        try:
            ...
            for s1, s2 in permutations(state_list, 2):
                tm[s1, s2] = full_tm[state_order_dict[f's{s1}'], state_order_dict[f'e{s1}_{s2}']]
            for s in state_list:
