...
from FretReportServer import FretReport
...
class Gui(object):
    ...
    def update_example_logprob(self):
        if all(self.hmm_obj.data.is_labeled):
            ...
            self.train_and_update()
            # print(self.hmm_obj.data.loc[np.invert(self.hmm_obj.data.is_labeled), 'logprob'])

            # min_logprob = self.hmm_obj.data.loc[np.invert(self.hmm_obj.data.is_labeled), 'logprob'].min()
            # self.cur_example_idx = self.hmm_obj.data.index[np.argwhere(self.hmm_obj.data.logprob == min_logprob).squeeze(1)[0]]
    ...
    def make_document(self, doc):
        ...
        # self.html_source.on_change('data', CustomJS(args=dict(file_source=self.html_source),
        #                                             code=download_report_impl))
